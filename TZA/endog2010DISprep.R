...
# households for 2010 at district level

setwd("C:/Users/Tomas/Documents/LEI/pol/data/link_files")

polLinkDis2010 <- readRDS("polLinkDis2010.rds") %>%
  select(y2_hhid, reg = REG.NAME, dis = DIS.NAME,
         ccm_prez10, split_prez10)

# -------------------------------------
# read in the official number of vouchers
# distributed to each region in 2010-11
# note that Dar-Es-Salaam does not receive
# any vouchers
...
HHA <- read_dta(file.path(dataPath, "HH_SEC_A.dta")) %>%
  select(y2_hhid, region, y2_rural)
...
endog2010 <- left_join(endog2010, polLinkDis2010)
...
# all the NA values belong to the islands
# and there should be no voucher totals for
# dar es salaam

endog2010 <- endog2010[!endog2010$region > 21,]

summary(glm(cbind(vouchAny, 1-vouchAny) ~ sex + age + log(vtot) + y2_rural +
              years + education + education1555 + N1555 +
              ccm_prez10*split_prez10, family=binomial, data=endog2010))

summary(glm(cbind(vouchAny, 1-vouchAny) ~ sex + age + log(vtot) +
              years + education + education1555 + N1555 + 
              ccm_prez10*split_prez10, family=binomial, data=endog2010[endog2010$y2_rural %in% 1,]))

# select only the maize farmers
dataPath <- "C:/Users/Tomas/Documents/LEI/data/TZA"
maize <- read_dta(file.path(dataPath, "TZNPS2AGRDTA/AG_SEC4A.dta")) %>%
  dplyr::select(y2_hhid, plotnum, zaocode) %>%
  group_by(y2_hhid) %>%
  summarise(maize = ifelse(any(zaocode %in% 11), 1, 0))

endog2010 <- left_join(endog2010, maize)
endog2010$y2_rural <- as.integer(endog2010$y2_rural)

# rural and maize farmers

summary(glm(cbind(vouchAny, 1-vouchAny) ~ sex + age + vtot +
              years + education + education1555 + N1555 + 
              split_prez10, family=binomial, data=endog2010[endog2010$y2_rural %in% 1 & endog2010$maize %in% 1,]))

hist(endog2010$split_prez10, freq=FALSE)
lines(density(endog2010$split_prez10))